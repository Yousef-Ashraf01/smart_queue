...
class ActiveBookingCubit extends Cubit<ActiveBookingState> {
  ...
  Future<void> loadFromPrefs() async {
    ...
    if (list.isEmpty) {
      ...
      if (slotStart != null &&
        ...
        final cancelledServiceIds = prefs.getStringList(BookingKeys.cancelledServiceIds) ?? [];
        final canCancel = serviceId == null || !cancelledServiceIds.contains(serviceId.toString());
    ...
    for (final item in list) {
      try {
        final Map<String, dynamic> booking = jsonDecode(item) as Map<String, dynamic>;
        ...
        booking['canCancel'] = serviceId == null || !cancelledServiceIds.contains(serviceId.toString());
  ...
  Future<void> removeBooking(int id) async {
    ...
    final state = this.state;
    if (state is ActiveBookingLoaded) {
      final updatedBookings = state.bookings.where((b) => b['id'] != id).toList();
  ...
  Future<void> cancelBooking(int id) async {
    ...
    final cancelledServiceIds = prefs.getStringList(BookingKeys.cancelledServiceIds) ?? [];
    if (serviceId != null && cancelledServiceIds.contains(serviceId.toString())) {
      emit(
        ActiveBookingError("You cannot cancel this service booking again. You are only allowed to cancel a service once."),
      );
      return;
    }
    ...
    result.fold((failure) => emit(ActiveBookingError(failure.message)), (
      ...
      if (serviceId != null) {
        ...
        await prefs.setStringList(BookingKeys.cancelledServiceIds, cancelledServiceIds);
      }
      ...
      // Update state
      final updatedBookings = currentBookings.where((b) => b['id'] != id).toList();
      if (updatedBookings.isEmpty) {
        emit(ActiveBookingInitial());
      } else {
        emit(ActiveBookingLoaded(updatedBookings));
