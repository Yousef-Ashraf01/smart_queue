...
class _TimerScreenState extends State<TimerScreen> {
  ...
  void _setupBookingTimer(Map<String, dynamic> booking) {
    ...
    // Remove any booking that is already expired before displaying anything
    final expiredIds = _remainingMap.entries
        .where((e) => e.value.inSeconds <= 0)
        .map((e) => e.key)
        .toList();
    for (final id in expiredIds) {
  ...
  void _startTicker() {
    ...
    _ticker = Timer.periodic(const Duration(seconds: 1), (_) {
      if (!mounted) return;
      final resolvedRemaining = remaining.isNegative ? Duration.zero : remaining;
    ...
    return BlocListener<ActiveBookingCubit, ActiveBookingState>(
      listener: (context, state) {
        ...
        if (state is ActiveBookingLoaded) {
          ...
          // Remove bookings that no longer exist in state
          final activeIds = state.bookings.map((b) => b['id'] as int?).whereType<int>().toSet();
          _remainingMap.removeWhere((id, _) => !activeIds.contains(id));
          _totalDurationMap.removeWhere((id, _) => !activeIds.contains(id));
        }
        ...
        builder: (context, state) {
          return Scaffold(
            body: Container(
              ...
              child: SafeArea(
                child: state is ActiveBookingLoaded && state.bookings.isNotEmpty
                    ? _buildMultiBookingView(state.bookings)
                    : _buildEmptyState(context),
              ),
            ),
                ...
                children: [
                  ...
                  Expanded(
                    child: ElevatedButton(
                      onPressed: () async {
                        Navigator.pop(context);
                        await context.read<ActiveBookingCubit>().cancelBooking(
                              booking['id'] as int,
                            );
                      },
                      style: ElevatedButton.styleFrom(
